...
request.get('https://random.dog/woof.json')
    .end((err, res) => {
      if (!err && res.status === 200) {
        ...
        console.log(`Erreur, ${err}, status code: ${res.status}`)
      }
    })	
	},
	'hug': (msg) => {
...
const rn = require('random-number')
    if (!msg.mentions.users.first()) return msg.channel.send("Bon, tu peux pas faire un calin dans le vide à part si tu es fou ... :neutral_face:");
    let r = rn({
        min: 0,
        max: hug.length - 1,
        integer: true
    ...
    let image = hug[r];
    msg.channel.send('**' + msg.author.username + '** *fait un calin à* **' + msg.mentions.users.first().username + '** :heart:\n' + image);
	}
